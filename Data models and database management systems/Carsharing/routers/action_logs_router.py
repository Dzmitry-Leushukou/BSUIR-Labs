...
def get_action_logs_endpoint(offset: int = 0, limit: int = 10, current_user: dict = Depends(get_current_user)):
    return get_action_logs(offset, limit)

@router.get("/count", response_model=dict)
def get_action_logs_count_endpoint(current_user: dict = Depends(get_current_user)):
    """
...
def get_action_log_endpoint(log_id: int, current_user: dict = Depends(get_current_user)):
    return get_action_log(log_id)

@router.post("/", response_model=ActionLog)
def create_action_log_endpoint(log: ActionLogCreate, current_user: dict = Depends(get_current_user)):
    return create_action_log(log)

@router.delete("/{log_id}")
def delete_action_log_endpoint(log_id: int, current_user: dict = Depends(get_current_user)):
