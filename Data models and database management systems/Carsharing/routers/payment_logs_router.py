...
def get_payment_logs_count_endpoint(current_user: dict = Depends(get_current_user)):
...
def create_payment_log_endpoint(log: PaymentLogCreate, current_user: dict = Depends(get_current_user)):
    return create_payment_log(log)

@router.delete("/{log_id}")
def delete_payment_log_endpoint(log_id: int, current_user: dict = Depends(get_current_user)):
