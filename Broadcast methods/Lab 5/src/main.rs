...
fn parse_cli_options(args: &[String]) -> Result<CliOptions, String> {
        ...
        match args[idx].as_str() {
            "--entry" => {
                let Some(value) = args.get(idx + 1) else {
                    return Err("после '--entry' нужно указать имя точки входа".to_string());
                };
                entry = value.clone();
                idx += 2;
            }
            ...
            option if option.starts_with("--") => {
                return Err(format!("неизвестный параметр '{}'", option));
            }
            path => {
                if filename.is_some() {
                    return Err(format!(
                        "путь к файлу уже задан, лишний аргумент '{}'",
                        path
                    ));
                }
    ...
    let Some(filename) = filename else {
        return Err("не указан путь к Idris-файлу".to_string());
    };
...
fn main() {
    ...
    let options = match parse_cli_options(&args) {
        ...
        Err(err) => {
            eprintln!(
                "Использование: {} <путь_к_idr_файлу> [--entry <имя>] [--eval-all] [--dump-bytecode]",
                args[0]
            );
            eprintln!("Ошибка параметров: {}", err);
            process::exit(1);
        }
    };

    let content = match fs::read_to_string(&options.filename) {
        Ok(value) => value,
        Err(err) => {
            eprintln!("Ошибка чтения '{}': {}", options.filename, err);
            process::exit(1);
        }
    };
    ...
    print_report(&options.filename, &syntax_report, &semantic_report);

    if !syntax_report.errors.is_empty() || !semantic_report.errors.is_empty() {
        process::exit(1);
    }

    let bytecode_program = interpreter::compile_to_bytecode(&syntax_report.assignments);
    interpreter::print_interpreter_build_report(&bytecode_program);

    if bytecode_program.has_errors() {
        process::exit(1);
    }

    if options.dump_bytecode {
        interpreter::print_bytecode(&bytecode_program);
    }

    let mut interpreter = interpreter::Interpreter::new(bytecode_program);
    let mut runtime_failed = false;

    if options.eval_all {
        let results = interpreter.evaluate_all_zero_arity();
        println!();
        println!("Результаты интерпретации (--eval-all):");
        if results.is_empty() {
            println!("- Нет определений с арностью 0 для вычисления");
        } else {
            for (name, value) in results {
                match value {
                    ...
                    Err(err) => {
                        runtime_failed = true;
                        println!("- {}: ошибка выполнения: {}", name, err);
                    }
                }
            }
        }
    } else if !interpreter.has_global(&options.entry) {
        println!();
        println!(
            "Интерпретация пропущена: точка входа '{}' не найдена. Можно использовать --eval-all",
            options.entry
        );
    } else {
        match interpreter.evaluate_entry(&options.entry) {
            Ok(value) => {
                println!();
                println!("Результат интерпретации ({}): {}", options.entry, value);
            }
            Err(err) => {
                runtime_failed = true;
                println!();
                println!(
                    "Ошибка выполнения точки входа '{}': {}",
                    options.entry, err
                );
            }
        }
