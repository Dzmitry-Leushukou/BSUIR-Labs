...
SERVICE_ID = os.getenv("SERVICE_ID", "printserver")
...
async def access_resource(request: ResourceRequest):
    ...
    logger.info(f"Print server request for service {SERVICE_ID}")
    ...
    response_text = f"Print server: Hello, {ticket['user_id']}! Your print job '{request.request_data}' is accepted."
    return {"response": response_text}

@app.get("/")
async def root():
    return {"service": "Print Server", "service_id": SERVICE_ID}

if __name__ == "__main__":
    logger.info(f"Starting Print Server on port {PORT}, KSS URL: {KSS_URL}")
