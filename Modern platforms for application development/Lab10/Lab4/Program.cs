...
builder.Services.AddScoped<Cart>(sp => new SessionCart(sp.GetRequiredService<IHttpContextAccessor>().HttpContext?.Session ?? throw new InvalidOperationException("Session not available")));
...
builder.Services
 ...
 // Ensure EndSessionEndpoint is set (important for logout)
 if (string.IsNullOrEmpty(loaded.EndSessionEndpoint))
 {
 loaded.EndSessionEndpoint = $"{realmBase}/protocol/openid-connect/logout";
 }
 ...
 // Verify EndSessionEndpoint is available
 Console.WriteLine($"DEBUG OIDC Config - EndSessionEndpoint: {conf?.EndSessionEndpoint ?? "NULL"}");
 Console.WriteLine($"DEBUG OIDC Config - Authority: {realmBaseUsed}");
 ...
 {
 // Keycloak calls this after user signs out on Keycloak side
 // Just let it redirect to SignedOutCallbackPath (/signout-callback-oidc)
 Console.WriteLine("DEBUG OnSignedOutCallbackRedirect called");
 return Task.CompletedTask;
 }
 };
 });
...
// Add error response logging middleware early in the pipeline
...
app.MapGet("/signout-callback-oidc", async ctx =>
{
 ...
 { 
  ...
  await ctx.SignOutAsync("keycloak"); 
 } 
 catch { }
 ...
 Console.WriteLine("DEBUG: signout-callback-oidc hit, clearing remaining cookies and redirecting to " + home);
...
{
    // Ensure Serilog is properly flushed and disposed
    Log.CloseAndFlush();
}
